...
\begin{align}
\hat{\pi}_{N, b}^{\mathsf{MC}}(f) = t(X^{(b)}_{1}, \ldots, X^{(b)}_{N})\,, \qquad b = 1, \ldots, B\,.
\end{align}
...
población desconocida y un estimador $\hat \theta_N=t(X_1,\dots, X_N)$.
...
*** Aclaración:
La notación hace enfásis en que la característica de la distribución
que nos interesa es resultado de aplicar un procedimiento numérico $t(\cdot)$ a
nuestra distribución $\mathbb{P}$. Esto no contradice nuestra notación. Pues
...
- Nuestros estimadores /bootstrap/ son estimadores
  \begin{align}
  \hat \theta_N^* = t(X^*_{1}, \ldots, X^*_{N})\,,
  \end{align}
...
#+begin_src R :exports code :results none :tangle no :eval never
  ...
  dist_boot <- datos_sim |>
    ...
    group_by(rep) |> nest() |> 
    mutate(precio_miles =  map(data, procedimiento_boostrap)) |>
