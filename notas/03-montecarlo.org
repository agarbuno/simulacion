...
#+attr_latex:
#+begin_exercise
Calcula la desigualdad y obtén el número de simulaciones necesarias para
...
estimación no diste mas de $\epsilon$ unidades del verdadero valor se alta ($1-\delta$).
