...
#+begin_src R :file images/dardos-trayectorias.jpeg :exports results :results output graphics file
  resultados |>
    ...
    scale_x_continuous(trans='log10', 
                       labels = trans_format("log10", math_format(10^.x))) + 
    ylab('Aproximación') + xlab("Muestras") + sin_lineas + sin_leyenda +
    ylim(0, 7)
#+end_src
...
distribuciones asintóticas, ($\mathsf{CLT}$).
...
*** ~Nota~:
El estimador Monte Carlo del que hablamos, $\hat \pi_{N}^{\mathsf{MC}}(f)$, es una estimación con una muestra finita de simulaciones. En ese sentido podemos pensar que tenemos un /mapeo/
\begin{align}
(x^{(1)}, \ldots, x^{(N)}) \mapsto  \hat \pi_N^{\mathsf{MC}}(f)\,,
\end{align}
con $x^{(i)} \overset{\mathsf{iid}}{\sim} \pi$ . 

De lo cual es natural pensar: ¿y si hubiéramos observado otro conjunto de simulaciones?
...
\begin{align}
\mathsf{ee}[\hat \pi_N^{\mathsf{MC}}(f)] = \left( \mathbb{V}[\hat \pi_N^{\mathsf{MC}}(f)]  \right)^{1/2}\,.
\end{align}

*** ~Nota~:
Para algunos estimadores la fórmula del error estándar se puede obtener de manera analítica (curso de ~Inferencia Matemática~). Para otro tipo, tenemos que utilizar propiedades asintóticas (p.e. cota de Cramer-Rao). 

Hay casos en los que no existe una fórmula asintótica o resultado analítico,
