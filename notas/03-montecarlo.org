...
\begin{align}
\mathsf{Prob} \left( |\hat \theta_N - \theta| < \epsilon \right) \geq 1 - \alpha\,.
\end{align}
...
\begin{align}
1 - \alpha \leq 1 - \frac{\mathbb{V}(\hat \theta_N)}{\epsilon^2}\,,
\end{align}

Del cual podemos despejar
\begin{align}
N \geq \frac{\theta (1 -\theta)}{\alpha \epsilon^2}\,.
\end{align}

*** ~Discusión~:
El resultado anterior nos permite escribir que con alta probabilidad (mayor a $1 -\alpha$) tendremos que 
\begin{align}
...
#+REVEAL: split
Una alternativa es utilizar la desigualdad de Hoeffding que nos permite acotar
